...
This pushes back on Osmani's framing in a way that deserves unpacking. The comprehension debt metaphor borrows its urgency from technical debt — but the analogy may be misleading. Technical debt slows you down *mechanically*: tangled dependencies, brittle interfaces, slow builds. The cost is concrete and recurring. Comprehension debt, by contrast, is a *latent* risk — it only materializes when something breaks in a way that tests don't catch and the AI can't diagnose. The question is how often that actually happens versus how often the alarm is raised.

The Vela case is instructive because it's not a toy. A 12K-line application with PDF export, AI integration, and real production use across dozens of sessions — evolved iteratively over five weeks without the developer ever needing to read most of the codebase. That's not "move fast and break things." That's *sustained velocity with zero comprehension and zero breakage*. The traditional model assumes comprehension is load-bearing — that without it, agility collapses under accumulated confusion. Vela suggests that for a class of applications, comprehension was never load-bearing at all. The AI handles local coherence (each change works), and the developer handles intent (what the app should do next). The gap between those two — system-level understanding — simply never becomes a bottleneck.

Nobody has ever understood 95% of the Linux kernel, V8, or React internals. We've always operated on trust layers — trusting compilers, frameworks, libraries we never read. AI-generated code extends that trust boundary inward, into *your own codebase*. The real question isn't "do you understand your code?" but "can you intervene when something breaks?" — and that's a narrower, more solvable problem.

Where the risk concentrates is in systems under active architectural evolution — where requirements shift the *structure*, not just the features. In those cases, comprehension debt can compound not because you can't read the code, but because you lose the ability to predict side effects of structural changes. Security and costly failures aren't edge cases there — they're the primary failure mode. So comprehension debt may be context-dependent: negligible for feature-driven applications like Vela, dangerous for systems where the architecture itself is the moving target.
